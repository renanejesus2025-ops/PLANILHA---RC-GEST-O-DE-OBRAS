...
## Registro de homologação — Revisão 11

Em 2026-09-23, construído o módulo **EXECUÇÃO/MEDIÇÕES** ("Etapa 7" de construção — Progresso Físico), refletido em:
- **EXECUÇÃO/MEDIÇÕES** — `Quantidade/% Executado` passa a ser validada como sempre não negativa (mesmo padrão de Quantidade Orçada/Valor). Interpretação de implementação registrada (não é regra de negócio nova): múltiplas medições do mesmo Serviço são ACUMULADAS (somadas), não substituídas — nenhuma fonte definiu o contrário.
- **SERVIÇOS/ORÇAMENTO** — os campos `metodo_execucao`/`status_execucao`/`peso_ajustado`/`peso_automatico_original` (já existentes desde a Etapa 1/3, nunca antes calculados) passam a ser efetivamente computados: motor de cálculo em `src/execucao/calculos.py`, refletido na aba Excel "Serviços" (colunas M–T) e na nova aba "Execução" (log de medições).
- **Cálculos que vivem em `src/execucao/calculos.py` (não são colunas de nenhuma entidade):** % de Execução do Serviço (REG-007); Peso Automático Bruto e Peso Efetivo (REG-008/023/024/025); % de Execução Física consolidado da Obra (REG-020); Peso Consolidado/Contribuição por Subetapa/Etapa (valores informativos de grupo — ver ressalva abaixo).
- **Decisão de implementação (REG-022):** elegibilidade usa o mesmo domínio já homologado `StatusServico.integra_orcamento_corrente` (Etapa 3) — não foi inventado um domínio novo.
- **Decisão de implementação (REG-024):** a fórmula técnica de redistribuição proporcional (marcada [H] em REG-024) foi implementada como proporcional ao Peso Automático Bruto de cada Serviço automático entre si — única leitura literal possível, sem outra base citada na regra.
- **Permanece [H], não inventado nesta rodada (REG-020):** um "% de execução isolado" por Subetapa/Etapa. Como o peso é sempre global (REG-008), não há fórmula homologada para expressar isso como um percentual 0–100% de um grupo isoladamente. Em vez disso, `Peso Consolidado (Execução)`/`Contribuição na Obra (Execução)` expõem valores diretos, não ambíguos — nunca chamados de "% da Subetapa/Etapa".
